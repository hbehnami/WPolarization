...
int main() {
  const double x = 0.1, Q = 3.3;

  #if defined LHAPDF_MAJOR_VERSION && LHAPDF_MAJOR_VERSION == 6
  LHAPDF::PDF* pdf = LHAPDF::mkPDF("cteq6l1", 0);
  std::cout << "xf_u = " << pdf->xfxQ(2, x, Q) << std::endl;
  std::cout << "xf_ubar = " << pdf->xfxQ(-2, x, Q) << std::endl;
  std::cout << "xf_d = " << pdf->xfxQ(1, x, Q) << std::endl;
  std::cout << "xf_dbar = " << pdf->xfxQ(-1, x, Q) << std::endl;
